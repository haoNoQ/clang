...
#include "clang/StaticAnalyzer/Core/Analyses.def"
  ...
  for (arg_iterator it = Args.filtered_begin(OPT_analyzer_checker_option),
         ie = Args.filtered_end(); it != ie; ++it) {
    const Arg *A = *it;
    A->claim();
    // We can have a list of comma separated checker names, e.g:
    // '-analyzer-checker=cocoa,unix'
    StringRef optionList = A->getValue();
    SmallVector<StringRef, 3> options;
    optionList.split(options, ":");
    llvm::StringMap<StringRef> &CheckerOpts =
        Opts.CheckerSpecificOptions[options[0]];
    CheckerOpts[options[1]] = options[2];
  }
